...
def _build_model(cfg: DictConfig) -> torch.nn.Module:
    ...
    if model_name == "mlp_baseline":
        return BaselineMLP(
            input_dim=cfg.model.get("input_dim", 15),
            hidden_dims=list(cfg.model.get("hidden_dims", [64, 32])),
            dropout=cfg.model.get("dropout", 0.1),
        )
    else:  # egnn / default
        return BindingGNN(
            input_node_dim=cfg.model.get("input_node_dim", PROTEIN_NODE_DIM),
            ligand_node_dim=cfg.model.get("ligand_node_dim", LIGAND_NODE_DIM),
            edge_attr_dim=cfg.model.get("edge_attr_dim", EDGE_ATTR_DIM),
...
def main(cfg: DictConfig) -> None:
    ...
    tracker.log_params({"n_params": n_params, "model_name": cfg.model.get("name")})
